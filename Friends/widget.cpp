    ...
    ui(new Ui::Widget)
{
    ...
    //私聊界面
    if(List::buttonflag==1)
    {
        ui->userTableWidget->deleteLater();
        ui->panda->setGeometry(470,76,301,361);
        QSize pandasize = ui->panda->size();
        QPixmap pandapixmap("://pic/panda.jpg");
        pandapixmap = pandapixmap.scaled(pandasize,Qt::IgnoreAspectRatio);
        ui->panda->setPixmap(pandapixmap);
    }
...
void Widget::sendMessage(messageType type, QString serverAddress)
{
    ...
    QString localHostName = QHostInfo::localHostName();//查找主机名
...
//QUdpSocket是QAbstractSocket的子类，它允许发送和接收UDP数据报。
//使用这个类最常见的方法是使用bind()绑定到地址和端口，然后调用writeDatagram()和readDatagram()来传输数据。
//如果想使用标准的QIODevice函数read()、readLine()、write()等，必须首先通过调用connectToHost()将套接字直接连接到对等端。
...
{
    while(udpSocket ->hasPendingDatagrams())//有数据等待被读取时
    {
        // qDebug() << "已链接上";
        QByteArray datagram;
        datagram.resize(udpSocket->pendingDatagramSize());//修改缓冲区size
...
QString Widget::getIP()
{
    ...
    foreach (QHostAddress address, list) {
        if(address.protocol() == QAbstractSocket::IPv4Protocol)//协议为IPv4时
...
QString Widget::getUserName()
{
    ...
    QStringList environment = QProcess::systemEnvironment();//获取系统环境变量
